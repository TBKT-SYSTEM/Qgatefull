...
namespace QGate_system
{
    public partial class qgateReprintDefect : Form
    {
        ...
        private async Task setComboBox()
        {
            ...
            try
            {
                ...
                TypeDefectItem itemTypeDefect0 = new TypeDefectItem("Choose TypeDefect", 0);
                cbTypeDefect.Items.Add(itemTypeDefect0);

                TypeDefectItem itemTypeDefect1 = new TypeDefectItem("NC",2);
                cbTypeDefect.Items.Add(itemTypeDefect1);

                TypeDefectItem itemTypeDefect2 = new TypeDefectItem("NG",1);
                cbTypeDefect.Items.Add(itemTypeDefect2);
        ...
        private void cbTypeDefect_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            TypeDefectItem selectedItem = (TypeDefectItem)cbTypeDefect.SelectedItem;
            string selectedtypeDefect = selectedItem.Index.ToString();
        ...
        private async void getDataTagDefect()
        {
            try
            {
                ...
                TypeDefectItem selectedItem = (TypeDefectItem)cbTypeDefect.SelectedItem;
                string selectedtypeDefect = selectedItem.Index.ToString();
                Console.WriteLine("ข้อความที่เลือก selectedtypeDefectK : " + selectedtypeDefect);
                ...
                else if (responsedataGetReprintDefect.Status == 10)
                {
    ...
    public class TypeDefectItem
    {
        public string Text { get; set; }
        public int Index { get; set; }

        public TypeDefectItem(string text, int index)
        {
            Text = text;
            Index = index;
        }

        // กำหนดให้แสดงเฉพาะ Text เมื่อเรียก ToString()
