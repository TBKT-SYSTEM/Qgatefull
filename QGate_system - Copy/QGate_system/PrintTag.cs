    ...
    public partial class PrintTag : Form
    {
        ...
        public void printTagQgate(string Sendto, string partno, string partNoName, string model, string custPartNo, string boxNo, DateTime Date, string tagqgate, string id_product, string location, string QTY, string lotno, string workshift, string line, string Phase)
        {
            ...
            printDoc.PrintPage += (sender, e) =>
            {
                ...
                //e.Graphics.DrawString("Test Print", new Font("Arial", 12), Brushes.Black, new PointF(10, 10));
                QRCodeGenerator generator = new QRCodeGenerator();
                ...
                e.Graphics.DrawString(formattedDate + " | " + qgateScanTag.genLot(Date), values.Font, Brushes.Black, 415, 213);
        ...
        private void ShowPrintPreview(PrintDocument printDoc)
        {
            if (this.InvokeRequired)
            {
                this.Invoke(new Action<PrintDocument>(ShowPrintPreview), printDoc);
            }
            else
            {
                try
