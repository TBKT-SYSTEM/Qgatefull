...
namespace QGate_system
{
    class memberData
    {
        ...
        public static bool removeLastMember()
        {
            ...
            //formSelectMenu.userSelected = "dfdfdfdfd";

            Console.WriteLine(formSelectMenu.userSelected);
            if (!string.IsNullOrEmpty(formSelectMenu.userSelected))
            {
                Console.WriteLine("userSelected != null");
                for (int i = memberList.Count - 1; i >= 0; i--)
                {
                    // ตรวจสอบว่าค่าใน index 0 ของสมาชิกใน memberList เท่ากับค่าของ formSelectMenu.userSelected หรือไม่
                    if (memberList[i][0] == formSelectMenu.userSelected)
                    {
                        // ถ้าเป็นเงื่อนไขนั้น ให้ลบสมาชิกที่ index นั้นออกจาก memberList
                        memberList.RemoveAt(i);
                    }
                }
                formSelectMenu.userSelected = "";
                return true;
            }
            else
            {
